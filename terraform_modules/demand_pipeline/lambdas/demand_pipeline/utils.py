...
def get_dynamo_table(table_name, account_id=None):
    ...
    :param local_mode: If True, always use local/default credentials and do not assume any role.
    ...
    try:
        ...
        if local_mode:
            ...
            print(f"Accessing table '{table_name}' in local mode with default credentials.")
            return table
        ...
        if SOURCE_ENV == "sandbox" or not account_id:
            dynamodb = boto3.resource("dynamodb")
            table = dynamodb.Table(table_name)
            print(f"Accessing table '{table_name}' in the *current* account (sandbox/default).")
            return table
        ...
        print(f"Accessing table '{table_name}' in account {account_id} via assumed role.")
        return table
...
def scan_dynamo_table(table, max_items=None):
    ...
    """
    if not table:
        print("Invalid table reference provided.")
        return []
        ...
        try:
            ...
            if max_items is not None:
                if total_processed + len(page_items) > max_items:
                    ...
                    print(f"Reached the maximum of {max_items} items.")
                    break
                else:
                    items.extend(page_items)
                    total_processed += len(page_items)
                    print(f"Scanned {total_processed} items so far...")
            else:
                items.extend(page_items)
                total_processed += len(page_items)
                print(f"Scanned {total_processed} items so far...")
            ...
            print(f"Error scanning table {table.table_name}: {e}")
            break
...
def parallel_scan_dynamo_table(
    ...
    def scan_segment(segment_index, total_segments, limit, max_items=None):
        ...
        while True:
            try:
                ...
                if e.response['Error']['Code'] == 'ProvisionedThroughputExceededException':
                    ...
                    print(f"Segment {segment_index}: Throughput exceeded. Sleeping for {sleep_time:.2f}s...")
        ...
        print(f"Segment {segment_index} finished scanning. Total items from this segment: {len(items)}")
        return items
    ...
    try:
        ...
        print(f"Error during parallel scan: {e}")
        return []
    ...
    print(f"Total items returned from parallel scan: {len(results)}")
    return results
...
def insert_data_into_table(conn, table_name, headers, data, save_csv=False, csv_file_path="output.csv"):
    ...
    try:
        ...
        conn.commit()
        print(f"Data successfully inserted into raw.{table_name}.")
    except Exception as e:
        conn.rollback()
...
def insert_data_and_validate(conn, table_name, headers, data):
    ...
    if inserted_count != source_count:
        raise ValueError(
            f"[{table_name}] Row count mismatch: source={source_count}, inserted={inserted_count}."
        )
    else:
        print(f"[{table_name}] Row count matched: {inserted_count} rows inserted.")
