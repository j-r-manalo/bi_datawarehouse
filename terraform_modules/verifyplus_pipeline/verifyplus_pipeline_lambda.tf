...
#############################################
data "aws_ssm_parameter" "verifyplus_layer_arn" {
  count = var.skip_layer_lookup ? 0 : 1
  name  = "/lambda-layers/verifyplus-${var.env}-layer/latest-arn"
}

# Default layer configuration for initial deployment
locals {
  verifyplus_pipeline_lambda_name = "${var.itc_database_prefix}-verifyplus-pipeline"
  verifyplus_layer_arn = var.skip_layer_lookup ? null : try(data.aws_ssm_parameter.verifyplus_layer_arn[0].value, null)
 ...
 # Create a list of layers with null handling
  lambda_layers = local.verifyplus_layer_arn != null ? [local.verifyplus_layer_arn, local.aws_sdk_pandas_arn] : [local.aws_sdk_pandas_arn]
}
...
#############################################
# Create secret for Quickbase token
#############################################
resource "aws_secretsmanager_secret" "quickbase_secret" {
  name = "${var.itc_database_prefix}-${var.env}-quickbase-token"
}
...
resource "aws_s3_bucket" "verifyplus_lambda_layer_bucket" {
  bucket = "${var.itc_database_prefix}-${var.env}-verifyplus-lambda-layer-bucket"  # Change to your desired bucket name
}
...
resource "aws_iam_role" "verifyplus_pipeline_lambda_role" {
  name = "${local.verifyplus_pipeline_lambda_name}-role" # must come before the lambda, but want to reference it's name
...
######################################
resource "aws_iam_policy" "verifyplus_pipeline_lambda_policy" {
  name = "${local.verifyplus_pipeline_lambda_name}-policy"
  policy = jsonencode({
    "Version" : "2012-10-17",
    "Statement" : concat(
      [
        ...
        },
        {
          ...
          Resource = [
            "arn:aws:s3:::${aws_s3_bucket.verifyplus_lambda_layer_bucket.bucket}",
            "arn:aws:s3:::${aws_s3_bucket.verifyplus_lambda_layer_bucket.bucket}/*"
          ]
        }
      ],
...
######################################
resource "aws_iam_role_policy_attachment" "verifyplus_pipeline_role_policy_attachment" {
  role       = aws_iam_role.verifyplus_pipeline_lambda_role.id
  policy_arn = aws_iam_policy.verifyplus_pipeline_lambda_policy.arn
}
...
data "archive_file" "verifyplus_pipeline_lambda_zip" {
  type        = "zip"
  source_dir  = "${path.module}/../verifyplus_pipeline/lambdas/verifyplus_pipeline"  # Adjust as needed.
  output_path = "${path.module}/../verifyplus_pipeline/lambdas/verifyplus_pipeline.zip"
...
#############################################
resource "aws_s3_object" "verifyplus_pipeline_lambda_code" {
  bucket = aws_s3_bucket.verifyplus_lambda_layer_bucket.id
  key    = "lambda_functions/verifyplus_pipeline.zip"
  source = data.archive_file.verifyplus_pipeline_lambda_zip.output_path
  etag   = filemd5(data.archive_file.verifyplus_pipeline_lambda_zip.output_path)
}
...
resource "aws_lambda_function" "verifyplus_pipeline_lambda" {
  # function_name = module.verifyplus_pipeline_lambda_package.function_name
  function_name = local.verifyplus_pipeline_lambda_name
  description   = "Lambda for the verifyplus Pipeline."
  ...
  s3_key           = aws_s3_object.verifyplus_pipeline_lambda_code.key
  source_code_hash = data.archive_file.verifyplus_pipeline_lambda_zip.output_base64sha256
